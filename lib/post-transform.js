...
export default function transform(src) {
  ...
  function getDocumentedParams(params, comment) {
    ...
    const knownParams =
      parseJSDoc(doc)
        .filter(tag => tag.name === 'param' || tag.name === 'this')
        .map(tag => tag.name === 'this' ? 'this' : tag.param.name);

    const remainingParams = knownParams.map((name, idx) => {
      const param = params[idx];

      if (!param) {
        throw new Error(`documented parameter <${ name }> at index <${idx}> not found`);
      }

      const actualName = param.argument?.name || param.name;

      if (actualName !== name) {
        throw new Error(`documented parameter <${ name }> at index <${idx}> differs from actual parameter <${ actualName }>`);
      }
  ...
  function fixOptionalArgsMethods(nodePath) {
    ...
    const {
      variations
    } = getDocumentedParams(params, commentPath.value).slice().reverse().reduce((res, param) => {
  ...
  function removeUnknownParams(nodePath) {
    ...
    const knownParams = getDocumentedParams(params.value, commentPath.value);
  ...
  function cleanComment(comment) {
    ...
    for (const tag of tags) {
      ...
      if (/class|constructor|template|method|typedef|property|this/.test(tag.name)) {
        replacements.push([ { start: tag.start - 4, end: tag.end } ]);
