...
export default function transform(src) {
  ...
  function keepCommentParams(commentPath, params) {
    ...
    return cleanComment(b.commentBlock.from({
      ...comment,
      value: newDoc
    }));
  }
  ...
  function fixOptionalArgsMethods(nodePath) {
    ...
    if (node.type !== 'TSDeclareMethod' && node.type !== 'TSDeclareFunction') {
      return;
    }

    const {
      variations
    } = node.params.slice().reverse().reduce((res, param) => {
      ...
      if (param.optional && required) {
        ...
        variations.forEach((variation) => {
          variation.push(b.identifier.from(param));
        });
      }
    ...
    const commentPath = commentPaths.get(commentPaths?.value.length - 1);
